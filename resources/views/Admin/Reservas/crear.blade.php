<div class="form-wrapper actividad-modal-wrapper">
  <div class="page-title">
    <i class="fa-solid fa-plus text-success" style="color:#28a745;"></i>
  ...
  @endif
  <form action="{{ route('admin.reservas.almacenar') }}" method="POST">
    @csrf
    <div class="form-grid-2cols">
      <div class="form-group">
        ...
        <select name="user_id" id="user_id" class="form-control" required>
          ...
          @foreach($clientes as $cliente)
            @if(!$cliente->is_admin)
              <option value="{{ $cliente->id }}">{{ $cliente->name }}{{ $cliente->apellidos ? ' ' . $cliente->apellidos : '' }}</option>
            @endif
          @endforeach
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="text" name="fecha" id="fecha" class="form-control" placeholder="Seleccione un día" autocomplete="off" disabled>
      </div>
      <div class="form-group">
        <label for="hueco"><i class="fas fa-clock icono-label"></i> Selecciona el hueco</label>
        <select name="cita_id" id="hueco" class="form-control" required disabled>
