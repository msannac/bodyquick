<div class="form-wrapper">
  ...
  @endif
  <form action="{{ route('cliente.perfil.actualizar') }}" method="POST" enctype="multipart/form-data" autocomplete="on">
    @csrf
    <div class="form-grid-2cols">
      <div class="form-group custom-file-wrapper-vertical">
        ...
        <input type="file" name="profile_photo" id="profile_photo" class="form-control file-narrow" accept="image/*" autocomplete="photo">
      </div>
      <div>
        <div class="form-group">
           <label for="name"><i class="fas fa-user icono-label"></i> Nombre</label>
           <input type="text" name="name" id="name" value="{{ $user->name }}" class="form-control" required autocomplete="given-name">
        </div>
        <div class="form-group">
           <label for="apellidos"><i class="fas fa-user-tag icono-label"></i> Apellidos</label>
           <input type="text" name="apellidos" id="apellidos" class="form-control" value="{{ $user->apellidos }}" autocomplete="family-name">
        </div>
        <div class="form-group">
           <label for="dni"><i class="fas fa-id-card icono-label"></i> DNI</label>
           <input type="text" name="dni" id="dni" class="form-control" value="{{ $user->dni }}" autocomplete="off">
        </div>
        <div class="form-group">
           <label for="telefono"><i class="fas fa-phone icono-label"></i> Teléfono</label>
           <input type="text" name="telefono" id="telefono" class="form-control" value="{{ $user->telefono }}" autocomplete="tel">
        </div>
        <div class="form-group">
           <label for="email"><i class="fas fa-envelope icono-label"></i> Correo electrónico</label>
           <input type="email" name="email" id="email" value="{{ $user->email }}" class="form-control" disabled required readonly autocomplete="email">
        </div>
        <div class="form-group">
